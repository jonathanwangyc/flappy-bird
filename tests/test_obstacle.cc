...
TEST_CASE("Constructor/InitializeGameMode() test") {
  SECTION("Game mode 1") {
    // set up a test obstacle instance with conditions of game mode 1 (easy)
    Obstacle obstacle = Obstacle(kWindowSizeX, kWindowSizeY, 1, vec2(0, 0), vec2(10, 10));

    // check if the variables are updated with the conditions for game mode 1
  ...
  SECTION("Game mode 2") {
    // set up a test obstacle instance with conditions of game mode 2 (normal)
    Obstacle obstacle = Obstacle(kWindowSizeX, kWindowSizeY, 2, vec2(0, 0), vec2(10, 10));

    // check if the variables are updated with the conditions for game mode 2
...
TEST_CASE("Obstacle AdvanceOneFrame() test") {
  ...
  vec2 one_frame = vec2(obstacle.GetMovingSpeed(), 0); // supposed shift of the obstacle after one frame
...
TEST_CASE("GenerateRandomDouble() test") {
  ...
  SECTION("One possible value") {
    // check the edge behavior of the function when min = max
